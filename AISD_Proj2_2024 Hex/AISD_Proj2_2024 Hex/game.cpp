...
void Game::reformatBoard()
{
	if (was_initialized) {
		...
		for (int i = board_size + 1; i < 2 * columns; i++) {
...
//	}
//}
const static bool isValid(int& row, int& col, int& rows, int& columns) {
	return row >= 0 && row < rows && col >= 0 && col < columns;
}
//bool Game::bfs(Vector<pair>& start_pos, char side) {
...
void resetVisited_multicore(bool** visited, int col, int rows, int columns) {

		for (int j = 0; j < rows; j++) {
			visited[j][col] = false;
		}
}
bool Game::dfs(Vector<pair>& start_pos, char side) {
	...
	while (sp!=0) {
		...
		if ((side == 'b' && current_cell == ',')) {
			...
			for (const auto& dir : directions) {
				...
				// Check if neighboring cell is within bounds, unvisited, and has the same color
				if (isValid(nr, nc, rows, columns))
					if (/*visited.count({nr,nc})==0*/ !visited[nr][nc])
						if (((board[nr][nc] == side) || board[nr][nc] == (side == 'r' ? 'x' : ','))) {
							stack[sp] = {nr, nc};
							sp++;
							visited[nr][nc] = true;
...
//				if (!visited[nr][nc])
//				{
//					char current_cell = board[nr][nc];
//					if (((current_cell == side) || current_cell == (side == 'r' ? 'x' : ','))) {
...
void Game::canWinInNMoves(int player, int moves)
{
	...
	//checking if the game is already over
